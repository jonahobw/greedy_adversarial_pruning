...
# Path to the model checkpoint to attack
model_path = (
...
# DataLoader parameters for the attack
# Adjust batch_size and num_workers as needed for your hardware
# pin_memory is typically True if using CUDA
# For demonstration, batch_size is set low

dl_kwargs = {"batch_size": 2, "pin_memory": False, "num_workers": 1}

# Parameters for the adversarial attack (PGD)
attack_params = {"eps": 2 / 255, "eps_iter": 0.001, "nb_iter": 5, "norm": np.inf}

# Set the dataset path environment variable for ShrinkBench
path = Path.cwd()
os.environ["DATAPATH"] = str(path / "datasets")

# Instantiate and run the attack experiment
# This will generate adversarial examples and evaluate the model's robustness

def main():
    """
    Run a PGD adversarial attack on a specified model checkpoint using ShrinkBench's AttackExperiment.
    """
    a = AttackExperiment(
